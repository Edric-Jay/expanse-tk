...
export default function AIInsightsPage() {
  ...
  const generateSmartSuggestions = (): SmartSuggestion[] => {
    ...
    if (totalIncome > 0) {
      ...
      if (currentSavings < recommendedSavings) {
        ...
        suggestions.push({
          ...
          description: `Your current savings rate is ${savingsRate.toFixed(1)}%. Try to save an additional ₱${Math.round(additionalSavingsNeeded).toLocaleString()} monthly to reach the recommended 20% savings rate.`,
    ...
    if (sortedCategories.length > 0) {
      ...
      if (categoryPercentage > 30 && topData.amount > 5000) {
        ...
        suggestions.push({
          ...
          description: `${topCategory} accounts for ${categoryPercentage.toFixed(1)}% of your expenses (₱${Math.round(topData.amount).toLocaleString()}). Consider reducing this by 15% to save ₱${potentialSavings.toLocaleString()} monthly.`,
    ...
    if (goals.length > 0) {
      const incompleteGoals = goals.filter((g) => g.current_amount < g.target_amount)

      if (incompleteGoals.length > 0) {
        const priorityGoal = incompleteGoals.reduce((prev, current) =>
          current.target_amount - current.current_amount < prev.target_amount - prev.current_amount ? current : prev,
        )

        const remainingAmount = priorityGoal.target_amount - priorityGoal.current_amount
        const monthsToComplete = Math.ceil(
          remainingAmount / (savingsRate > 0 ? (totalIncome * savingsRate) / 100 : 1000),
        )

        suggestions.push({
          id: suggestionId++,
          title: `Accelerate "${priorityGoal.name}" Goal`,
          description: `You need ₱${Math.round(remainingAmount).toLocaleString()} more to complete this goal. At your current savings rate, it will take ${monthsToComplete} months. Consider allocating ₱${Math.round(remainingAmount / 6).toLocaleString()} monthly to complete it in 6 months.`,
          priority: "medium",
          effort: "low",
          category: "Goals",
    ...
    if (wallets.length > 1) {
      ...
      const totalWalletBalance = walletsWithBalance.reduce((sum, w) => sum + w.balance, 0)

      if (walletsWithBalance.length > 2 && totalWalletBalance > 10000) {
        suggestions.push({
          id: suggestionId++,
          title: "Consolidate Wallet Balances",
          description: `You have ₱${Math.round(totalWalletBalance).toLocaleString()} spread across ${walletsWithBalance.length} wallets. Consider consolidating smaller balances into your main wallet for better tracking and potential higher interest earnings.`,
          priority: "low",
          effort: "low",
    ...
    if (!emergencyFundGoal && totalExpenses > 0) {
      suggestions.push({
        ...
        description: `Build an emergency fund of ₱${Math.round(recommendedEmergencyFund).toLocaleString()} (3 months of expenses). Start with ₱${Math.round(recommendedEmergencyFund / 12).toLocaleString()} monthly to build it over a year.`,
        priority: "high",
        effort: "medium",
    ...
    if (expenses.length > 10) {
      ...
      const avgDailySpending = recentExpenses.reduce((sum, t) => sum + Math.abs(t.amount), 0) / 30
      ...
      if (projectedMonthlySpending > totalExpenses * 1.1) {
        ...
        suggestions.push({
          ...
          description: `Your recent spending pattern shows ₱${Math.round(avgDailySpending).toLocaleString()} daily average, which projects to ₱${Math.round(projectedMonthlySpending).toLocaleString()} monthly. This is higher than your usual spending. Review recent transactions for any unusual expenses.`,
          priority: "medium",
          effort: "low",
          category: "Monitoring",
          icon: TrendingUp,
    ...
    const smallExpenses = expenses.filter((t) => Math.abs(t.amount) < 1000 && Math.abs(t.amount) > 100)
    if (smallExpenses.length > 10) {
      ...
      suggestions.push({
        ...
        description: `You have ${smallExpenses.length} small expenses totaling ₱${Math.round(totalSmallExpenses).toLocaleString()}. Review these for subscriptions or services you might not need. Canceling just a few could save ₱${Math.round(totalSmallExpenses * 0.2).toLocaleString()} monthly.`,
        priority: "low",
        effort: "low",
  ...
  }, [transactionsLoading, walletsLoading, goalsLoading, transactions, wallets, goals])
  ...
  const generateInsights = () => {
    ...
    if (topCategory !== "None" && totalExpenses > 0) {
      ...
      if (percentage > 40) {
        generatedInsights.push({
          ...
          description: `${topCategory} makes up ${percentage.toFixed(1)}% of your total expenses. Consider reviewing this category for potential savings.`,
          impact: "medium",
          category: topCategory,
          icon: AlertTriangle,
          color: "yellow",
    ...
    if (wallets.length > 1) {
      const highestBalanceWallet = wallets.reduce((prev, current) => (prev.balance > current.balance ? prev : current))
      generatedInsights.push({
        id: 4,
        type: "optimization",
  ...
  const generateAIResponse = (message: string) => {
    ...
    if (lowerMessage.includes("budget") || lowerMessage.includes("spending")) {
      return `Based on your data, your total expenses are ₱${totalExpenses.toLocaleString()}. Your highest spending categories are ${insights[0]?.category || "Food"} and ${insights[1]?.category || "Transportation"}. I recommend setting a monthly budget for each category to better control your spending.`
    }

    if (lowerMessage.includes("save") || lowerMessage.includes("goal")) {
      return `You currently have ${goals.length} financial goals set up. Your total savings are ₱${(totalIncome - totalExpenses).toLocaleString()}. To optimize your savings: 1) Automate transfers to avoid spending temptation, 2) Consider the 50/30/20 rule, and 3) Review and increase savings by 1% each month.`
    }

    if (lowerMessage.includes("investment") || lowerMessage.includes("invest")) {
      return `You have ₱${totalBalance.toLocaleString()} across ${wallets.length} wallets. For investments, consider: 1) Moving some funds to a high-yield savings account for better returns, 2) Starting with low-risk index funds if you're new to investing, 3) Diversifying across different asset classes. Always invest only what you can afford to lose and maintain your emergency fund first.`
    }

    return `I understand you're asking about your finances. Based on your current data, you have ₱${totalBalance.toLocaleString()} total balance across ${wallets.length} wallets. Your monthly savings rate is ${totalIncome > 0 ? (((totalIncome - totalExpenses) / totalIncome) * 100).toFixed(1) : 0}%. Is there a specific area you'd like me to analyze deeper - spending patterns, savings optimization, or goal planning?`
  }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Get personalized recommendations and smart financial advice</p>
          </div>
        </div>
                    ...
                    insights.map((insight) => (
                      <div key={insight.id} className="p-4 rounded-lg border hover:bg-gray-50 transition-colors">
                        <div className="flex items-start gap-3">
                          <div className={`p-2 rounded-full bg-${insight.color}-100`}>{getInsightIcon(insight)}</div>
                      ...
                      <div key={chat.id} className={`flex ${chat.type === "user" ? "justify-end" : "justify-start"}`}>
                        <div
                          className={`max-w-[80%] p-3 rounded-lg ${
                            chat.type === "user" ? "bg-blue-600 text-white" : "bg-white border"
                          }`}
                        >
                ...
                <div className="space-y-4">
                  {smartSuggestions.length > 0 ? (
                    smartSuggestions.map((suggestion) => (
                      <div key={suggestion.id} className="p-3 rounded-lg border hover:bg-gray-50 transition-colors">
                    ...
                    <div className="text-center py-8 text-gray-500">
                      <p className="text-sm">Add more financial data to get personalized suggestions.</p>
                    </div>
                  )}
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm text-gray-600">
                    {healthScore >= 80 ? "Excellent" : healthScore >= 60 ? "Good" : "Needs improvement"} financial
                    health
                  </p>
                  ...
                  <div className="space-y-3 text-left">
                    <div className="flex justify-between text-sm">
                      ...
                      <span
                        className={
                          totalIncome > 0 && ((totalIncome - totalExpenses) / totalIncome) * 100 >= 20
                            ? "text-green-600"
                            : "text-yellow-600"
                        }
                      >
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={goals.length > 0 ? "text-green-600" : "text-yellow-600"}>
              ...
              <CardContent>
                <div className="space-y-3 text-sm">
                  <div className="flex items-center gap-2 text-green-600">
                    ...
                    <span>
                      {wallets.length > 0
                        ? `Managing ${wallets.length} wallet${wallets.length > 1 ? "s" : ""} successfully`
                  ...
                  <div className="flex items-center gap-2 text-green-600">
                    ...
                    <span>
                      {goals.length > 0
                        ? `${goals.length} financial goal${goals.length > 1 ? "s" : ""} set`
