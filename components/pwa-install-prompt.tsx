...
export function PWAInstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (dismissed && dismissedTime) {
      ...
      if (daysSinceDismissed < 1) {
        // Show again after 1 day instead of 7
    ...
    // For iOS or if no beforeinstallprompt event, show manual install instructions
    ...
    // For testing: show prompt after 3 seconds regardless
  ...
  // Debug: Force show prompt (remove this in production)
  if (process.env.NODE_ENV === "development") {
    return (
      <div className="fixed top-4 right-4 z-50">
        <button onClick={() => setShowPrompt(true)} className="bg-blue-600 text-white px-3 py-1 rounded text-sm">
          Test Install Prompt
        </button>
        {showPrompt && (
